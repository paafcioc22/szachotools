...
namespace App2.View.TworzPaczki
{
    ...
    public partial class CreatePaczkaListaPaczek : ContentPage
    {
        ...
        private async void Button_Clicked(object sender, EventArgs e)
        {
            ...
            var paczka=await reposytorySQL.SavePaczkaToBase(new FedexPaczka(), this.fmm_gidnumer, 0);
            await DisplayAlert("Pamiętaj..", $"Oznacz karton(y) numerem {paczka.Fmm_EleNumer}","OK");
            GetListaPaczek(this.fmm_gidnumer);
        ...
        bool CanDeletePaczka(int numerPaczki)
        {
            bool tmp = false;

            if(Items.Count==numerPaczki)
                tmp= true;
            return tmp;
        }
        private  async void MenuItem_Clicked(object sender, EventArgs e)
        {
            ...
            if (CanDeletePaczka(usunKarton.Fmm_EleNumer))
            {
                var action = await DisplayAlert(null, $"Czy chcesz usunąć karton nr {usunKarton.Fmm_EleNumer} z listy?", "Tak", "Nie");
                if (action)
                {
            ...
            {
                await DisplayAlert("Uwaga", "Aby utrzymać porządek w numeracji możesz usunąć tylko ostatni karton z listy", "OK");
            }
